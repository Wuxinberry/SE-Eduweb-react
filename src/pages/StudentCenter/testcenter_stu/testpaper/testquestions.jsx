...
class StudentCenter extends React.Component {
  ...
  handuppaper=()=>{
    axios
              .get('http://127.0.0.1:8000/addanswerpaper/'+this.state.examinfo[0]['paper_id']+'/'+this.state.examinfo[0]['exam_id']+'/1',
                    { 
                      headers:{'content-type':'application/x-www-form-urlencoded'},
    ...
    for(let i=0;i<choosenum;i++){
      axios
              .get('http://127.0.0.1:8000/addanswerchoose/'+this.state.examinfo[0]['paper_id']+'/'+this.state.examinfo[0]['exam_id']+'/1/'+datasourcechoose[i]['cid']+'/'+datasourcechoose[i]['select']+'/'+datasourcechoose[i]['value'],
                    { 
                      headers:{'content-type':'application/x-www-form-urlencoded'},
    ...
    for(let i=0;i<judgenum;i++){
      axios
              .get('http://127.0.0.1:8000/addanswerjudge/'+this.state.examinfo[0]['paper_id']+'/'+this.state.examinfo[0]['exam_id']+'/1/'+datasourcejudge[i]['jid']+'/'+datasourcejudge[i]['select']+'/'+datasourcejudge[i]['value'],
                    { 
                      headers:{'content-type':'application/x-www-form-urlencoded'},
    ...
    axios
              .get('http://127.0.0.1:8000/exammodifystate/'+this.state.examinfo[0]['exam_id'],
                    { 
                      headers:{'content-type':'application/x-www-form-urlencoded'},
    ...
    return (
      <Layout>
        ...
        <Layout className="site-layout" style={{color:'white', marginLeft: 200 }}>
            ...
            <Button type='primary' disabled={aable} onClick={this.handuppaper}style={{ marginLeft: 20 ,marginRight:500 }} >交卷</Button>
            <Divider />
